  ...
  82                     ; 134 void TSL_Init(void)
  82                     ; 135 {
  84                     .text:	section	.text,new
  85  0000               _TSL_Init:
  89                     ; 137   disableInterrupts();
  92  0000 9b            sim
  94                     ; 139   DetectionIntegrator = DETECTION_INTEGRATOR_DEFAULT;
  97  0001 35020009      	mov	_DetectionIntegrator,#2
  98                     ; 140   EndDetectionIntegrator = END_DETECTION_INTEGRATOR_DEFAULT;
 100  0005 35020008      	mov	_EndDetectionIntegrator,#2
 101                     ; 141   ECSTimeStep = ECS_TIME_STEP_DEFAULT;
 103  0009 35140006      	mov	_ECSTimeStep,#20
 104                     ; 142   ECSTemporization = ECS_TEMPO_DEFAULT;
 106  000d 35140005      	mov	_ECSTemporization,#20
 107                     ; 143   RecalibrationIntegrator = RECALIBRATION_INTEGRATOR_DEFAULT;
 109  0011 351e0007      	mov	_RecalibrationIntegrator,#30
 110                     ; 144   DetectionTimeout = DTO_DEFAULT;
 112  0015 725f000a      	clr	_DetectionTimeout
 113                     ; 146   ECS_K_Fast = ECS_IIR_KFAST_DEFAULT;
 115  0019 35140004      	mov	_ECS_K_Fast,#20
 116                     ; 147   ECS_K_Slow = ECS_IIR_KSLOW_DEFAULT;
 118  001d 350a0003      	mov	_ECS_K_Slow,#10
 119                     ; 148   ECSTimeStepCounter = ECSTimeStep;
 121  0021 35140000      	mov	_ECSTimeStepCounter,#20
 122                     ; 149   ECSTempoCounter = 0;
 124  0025 725f0001      	clr	_ECSTempoCounter
 125                     ; 150   ECSTempoPrescaler = 0;
 127  0029 725f0000      	clr	_ECSTempoPrescaler
 128                     ; 152   TSL_IO_Init();
 130  002d cd0000        	call	_TSL_IO_Init
 132                     ; 154   TSL_Timer_Init();
 134  0030 cd0000        	call	_TSL_Timer_Init
 136                     ; 156   TSL_SCKey_Init();
 138  0033 cd0000        	call	_TSL_SCKey_Init
 140                     ; 162   enableInterrupts();
 143  0036 9a            rim
 145                     ; 164   TSLState = TSL_IDLE_STATE;
 148  0037 3501000e      	mov	_TSLState,#1
 149                     ; 166 }
 152  003b 81            	ret
 188                     ; 179 void TSL_Action(void)
 188                     ; 180 {
 189                     .text:	section	.text,new
 190  0000               _TSL_Action:
 194                     ; 182   switch ( TSLState )
 ...
 210  0011               L53:
 211                     ; 185     case TSL_IDLE_STATE:
 211                     ; 186       TSLState = TSL_SCKEY_P1_ACQ_STATE;
 213  0011 3502000e      	mov	_TSLState,#2
 214                     ; 187       break;
 216  0015 203e          	jra	L16
 217  0017               L73:
 218                     ; 190       disableInterrupts();
 221  0017 9b            sim
 223                     ; 191       Local_TickFlag.b.DTO_1sec = TSL_Tick_Flags.b.DTO_1sec;
 226                     	btst		_TSL_Tick_Flags,#0
 227  001d 90110001      	bccm	_Local_TickFlag,#0
 228                     ; 192       TSL_Tick_Flags.b.DTO_1sec = 0;
 230  0021 72110000      	bres	_TSL_Tick_Flags,#0
 231                     ; 193       enableInterrupts();
 234  0025 9a            rim
 236                     ; 195       TSL_SCKEY_P1_Acquisition();
 239  0026 cd0000        	call	_TSL_SCKEY_P1_Acquisition
 241                     ; 196       TSLState = TSL_SCKEY_P1_PROC_STATE;
 243  0029 3503000e      	mov	_TSLState,#3
 244                     ; 197       break;
 246  002d 2026          	jra	L16
 247  002f               L14:
 248                     ; 199     case TSL_SCKEY_P1_PROC_STATE:
 248                     ; 200       for ( KeyIndex = 0; KeyIndex < SCKEY_P1_KEY_COUNT; KeyIndex++ )
 250  002f 3f08          	clr	_KeyIndex
 251  0031               L36:
 252                     ; 202         TSL_SCKey_Process();
 254  0031 cd0000        	call	_TSL_SCKey_Process
 256                     ; 200       for ( KeyIndex = 0; KeyIndex < SCKEY_P1_KEY_COUNT; KeyIndex++ )
 258  0034 3c08          	inc	_KeyIndex
 261  0036 b608          	ld	a,_KeyIndex
 262  0038 a103          	cp	a,#3
 263  003a 25f5          	jrult	L36
 264                     ; 211       TSLState = TSL_ECS_STATE;
 266  003c 350b000e      	mov	_TSLState,#11
 267                     ; 214       break;
 269  0040 2013          	jra	L16
 270  0042               L34:
 271                     ; 285     case TSL_ECS_STATE:
 271                     ; 286       TSL_ECS();
 273  0042 cd0000        	call	_TSL_ECS
 275                     ; 287       TSL_GlobalSetting.whole = TSL_TempGlobalSetting.whole;
 277  0045 be04          	ldw	x,_TSL_TempGlobalSetting
 278  0047 bf0c          	ldw	_TSL_GlobalSetting,x
 279                     ; 288       TSL_TempGlobalSetting.whole = 0;
 281  0049 5f            	clrw	x
 282  004a bf04          	ldw	_TSL_TempGlobalSetting,x
 283                     ; 289       TSL_GlobalState.whole = TSL_TempGlobalState.whole;
 285  004c 45030b        	mov	_TSL_GlobalState,_TSL_TempGlobalState
 286                     ; 290       TSL_TempGlobalState.whole = 0;
 288  004f 3f03          	clr	_TSL_TempGlobalState
 289                     ; 291       TSLState = TSL_IDLE_STATE;
